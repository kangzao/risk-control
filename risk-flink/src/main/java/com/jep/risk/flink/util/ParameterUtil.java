...
import com.jep.risk.common.exception.custom.FlinkPropertiesException;
import com.jep.risk.common.exception.enums.FlinkPropertiesExceptionInfo;
...
public class ParameterUtil {
            ...
            return ParameterTool
                    ...
                    .fromPropertiesFile(
                            ...
                            Thread.currentThread()
                                    //返回该线程的上下文信息, 获取类加载器
                                    .getContextClassLoader().getResourceAsStream(envActive))
                    // 从启动参数中获取配置
    ...
    public static void envWithConfig(StreamExecutionEnvironment env, ParameterTool parameterTool) {
        ...
        env.getCheckpointConfig().setCheckpointTimeout(parameterTool.getInt(ParameterConstants.FLINK_CHECKPOINT_TIMEOUT));
        // checkpoint 语义设置为 精确一致( EXACTLY_ONCE )
        env.getCheckpointConfig().setCheckpointingMode(CheckpointingMode.EXACTLY_ONCE);
        // 最多允许 checkpoint 失败 3 次
        env.getCheckpointConfig().setTolerableCheckpointFailureNumber(parameterTool.getInt(ParameterConstants.FLINK_CHECKPOINT_FAILURENUMBER));
        // 同一时间只允许一个 checkpoint 进行
        env.getCheckpointConfig().setMaxConcurrentCheckpoints(parameterTool.getInt(ParameterConstants.FLINK_CHECKPOINT_MAXCONCURRENT));
        //设置 State 存储
